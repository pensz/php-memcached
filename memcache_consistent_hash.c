...
static void mmc_consistent_populate_buckets(mmc_consistent_state_t *state) /* {{{ */
{
	...
	for (i=0; i<state->num_points; i++) {
		printf("mc hash is %s point is %u \n", state->points[i].server->hash_str, state->points[i].point, state->points[i].server->port);
	}
...
void mmc_consistent_add_server(void *s, mmc_t *mmc, unsigned int weight) /* {{{ */
{
	...
	char *key = emalloc(strlen(mmc->hash_str) + MAX_LENGTH_OF_LONG + 2);
	...
	for (i=0; i<points; i++) {
		key_len = sprintf(key, "%s-%d", mmc->hash_str, i);
