	...
	for (i=0,j=0,path_len=strlen(save_path); i<path_len; i=j+1) {
		...
		if (i < j) {
			...
			if (url->scheme && url->path && !strcmp(url->scheme, "file")) {
				...
				if (persistent) {
					mmc = mmc_find_persistent(host, host_len, 0, timeout, retry_interval, 0, 0 TSRMLS_CC);
				}
				else {
					mmc = mmc_server_new(host, host_len, 0, 0, timeout, retry_interval, 0, 0 TSRMLS_CC);
				}
				...
				if (persistent) {
					mmc = mmc_find_persistent(url->host, strlen(url->host), url->port, timeout, retry_interval, 0, 0 TSRMLS_CC);
				}
				else {
					mmc = mmc_server_new(url->host, strlen(url->host), url->port, 0, timeout, retry_interval, 0, 0 TSRMLS_CC);
				}
			}
