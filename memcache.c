...
mmc_t *mmc_server_new(char *host, int host_len, unsigned short port, int persistent, int timeout, int retry_interval, 
		char *hash_str, int hash_str_len TSRMLS_DC) /* {{{ */
{
	...
	if (hash_str && hash_str_len) {
		mmc->hash_str = pemalloc(hash_str_len + 1, persistent);
		memcpy(mmc->hash_str, hash_str, hash_str_len);
		mmc->hash_str[hash_str_len] = '\0';
	} else {
		mmc->hash_str = pemalloc(host_len + MAX_LENGTH_OF_LONG + 1 + 1, persistent);
		sprintf(mmc->hash_str, "%s:%d", host, port);
	}
...
static void php_mmc_connect (INTERNAL_FUNCTION_PARAMETERS, int persistent) /* {{{ */
{
	...
	if (persistent) {
		mmc = mmc_find_persistent(host, host_len, port, timeout, MMC_DEFAULT_RETRY, 0, 0 TSRMLS_CC);
	}
	else {
		MMC_DEBUG(("php_mmc_connect: creating regular connection"));
		mmc = mmc_server_new(host, host_len, port, 0, timeout, MMC_DEFAULT_RETRY, 0, 0 TSRMLS_CC);
	}
...
PHP_FUNCTION(memcache_add_server)
{
	...
	int resource_type, host_len, list_id, hash_str_len;
	char *host, *hash_str;

	if (mmc_object) {
		if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|lblllbzls", &host, &host_len, &port, &persistent, &weight, &timeout, &retry_interval, &status, &failure_callback, &timeoutms, &hash_str, &hash_str_len) == FAILURE) {
			return;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "Os|lblllbzls", &mmc_object, memcache_class_entry_ptr, &host, &host_len, &port, &persistent, &weight, &timeout, &retry_interval, &status, &failure_callback, &timeoutms, &hash_str, &hash_str_len) == FAILURE) {
			return;
		}
	}
	...
	if (persistent) {
		mmc = mmc_find_persistent(host, host_len, port, timeout, retry_interval, hash_str, hash_str_len TSRMLS_CC);
	}
	else {
		MMC_DEBUG(("memcache_add_server: initializing regular struct"));
		mmc = mmc_server_new(host, host_len, port, 0, timeout, retry_interval, hash_str, hash_str_len TSRMLS_CC);
	}
...
mmc_t *mmc_find_persistent(char *host, int host_len, int port, int timeout, int retry_interval, char *hash_str, int hash_str_len TSRMLS_DC) /* {{{ */
{
	mmc_t *mmc;
	...
	if (zend_hash_find(&EG(persistent_list), hash_key, hash_key_len+1, (void **) &le) == FAILURE) {
		...
		mmc = mmc_server_new(host, host_len, port, 1, timeout, retry_interval, hash_str, hash_str_len TSRMLS_CC);
